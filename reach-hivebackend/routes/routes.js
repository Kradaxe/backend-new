...
router.post("/upload", authMiddleware, async (req, res) => {
  ...
  const  shareableToken = makeid(10);
  console.log(shareableToken);
  const newPost = new post({
    photo: result.url,
    userId: req.userId,
    shareToken:shareableToken
  });
...
router.get("/posts/:shareToken", async (req, res) => {
  try {
    const images = await post.findOne({ shareToken:req.params.shareToken});
    console.log(images);
    res.status(201).json({
      success: true,
      body: images,
